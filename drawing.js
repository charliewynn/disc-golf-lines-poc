...
var lightGreenFill = "rgba(34,239,34,.61)";
...
function getTextWidth(text, font) {
  context.font =
    typeof font == "undefined" ? "italic bold 15px sans-serif" : font;
...
function redraw() {
  ...
  if (!savedSinceLastStop && !flying) {
    ...
    for (b in breadCrumbs) {
      drawRect(
        ...
        4,
        4,
        breadCrumbsColor(breadCrumbsColorNDX)
      );
    }
    drawArrow(
      player.startPos.x,
      player.startPos.y,
      player.startPos.x + player.lastLaunchDir.x,
      player.startPos.y + player.lastLaunchDir.y,
      breadCrumbsColor(breadCrumbsColorNDX)
    );
  ...
  if (breadCrumbsImg) context.drawImage(breadCrumbsImg, 0, 0);

  for (img in breadCrumbsCanvases) {
    if (img < breadCrumbsCanvases.length - 4) {
      if (breadCrumbsCanvases[img]) {
        drawCanvasToCurrentCanvas(breadCrumbsCanvases[img], 0.35);
      ...
      var alpha = 1 - (breadCrumbsCanvases.length - (1 + parseInt(img))) * 0.12;
      drawCanvasToCurrentCanvas(breadCrumbsCanvases[img], alpha);
    }
  }
  if (currBreadCrumbsImg) context.drawImage(currBreadCrumbsImg, 0, 0);
  ...
  for(p in gravMapPts)
  {
    var fill = "rgba(100,100,100,1)"
    drawRect(gravMapPts[p].pos.add(gravMapPts[p].dir).x-4, gravMapPts[p].pos.add(gravMapPts[p].dir).y-4, 8,8, fill);
    context.beginPath()
    context.lineWidth = 3;
    context.strokeStyle = fill;
    context.moveTo(gravMapPts[p].pos.x, gravMapPts[p].pos.y);
    context.lineTo(gravMapPts[p].pos.add(gravMapPts[p].dir).x, gravMapPts[p].pos.add(gravMapPts[p].dir).y);
    context.stroke();
    
		//drawArrow(gravMapPts[p].pos.x, gravMapPts[p].pos.y, gravMapPts[p].pos.add(gravMapPts[p].dir).x, gravMapPts[p].pos.add(gravMapPts[p].dir).y, fill);
  }*/

  for (
    var b = Math.max(0, breadCrumbs.length - numBreadCrumbsDraw);
    b < breadCrumbs.length;
    ++b
  ) {
    drawRect(
      breadCrumbs[b].x,
  ...
  for (p in planets) planets[p].draw();
  player.draw();
  ...
  if (dragging) {
    for (var fakePlayer = 0; fakePlayer < 10; fakePlayer++) {
      ...
      newPlayer.fade = player.fade;
      newPlayer.throwStartEnergy = player.throwStartEnergy;
      newPlayer.energy = newPlayer.throwStartEnergy;
      for (var i = 0; i < 60; i++) {
  ...
  if (dragging || (!flying && !player.crashed)) {
    ...
    drawText(
      mousept.x - 10,
      mousept.y + 10,
      Math.floor(player.direction.x) + "," + Math.floor(player.direction.y),
      whiteFill,
      "italic bold 15px sans-serif"
    );
  }

  if (
    player.position.x < screenLeft - player.radius &&
    player.position.y > screenTop + player.radius &&
    player.position.y < screenBottom - player.radius
  )
    drawArrow(
      screenLeft + 20,
      player.position.y,
  ...
  if (
    player.position.x > screenRight - player.radius &&
    player.position.y > screenTop + player.radius &&
    player.position.y < screenBottom - player.radius
  )
    drawArrow(
  ...
  var text =
    "Drag across the screen to set launch direction/speed. Release to launch";
  ...
  drawText(
    5,
    height - 55,
    "Launch Attempts : " + breadCrumbsColorNDX,
    whiteFill,
    "italic bold 25px sans-serif"
  );
  drawText(
    5,
    height - 30,
    "Last Launch Direction : " +
      player.lastLaunchDir.x.toFixed(0) +
      "," +
      player.lastLaunchDir.y.toFixed(0),
    whiteFill,
    "italic bold 25px sans-serif"
  );
  drawText(
    5,
    height - 5,
    "Current Distance Traveled: " + player.distTraveled.toFixed(1),
    whiteFill,
    "italic bold 25px sans-serif"
  );
  ...
  if (player.arrivedAtGoal && message == undefined) {
    if (currentLevel == levels.length - 1) {
      message = new Message(); //= "Nice Job, You won. Click to Start Over";
    } else {
      message = new Message(); //= "Nice Job, You won. Click the screen to go to next level";
    }
  }

  if (message != undefined) message.draw();
}
...
function drawTextPos(text, textWidth, y) {
  ...
  drawText(offSet, y + fontSize, text, whiteFill, font);
}
