...
function updateGame()
{
  ...
  if(particles.length < numParticles)
  {
    ...
    if(flying)
      ...
      var actualPlanets = [];
      for(i in planets)
        if(!planets[i].isGoal && !planets[i].isBonus)
          actualPlanets.push(i);
          
      
      var planetNDX = actualPlanets[Math.floor(Math.random()*actualPlanets.length)];
      ...
      particles.push({pos:pos.add(whichPlanet.position),
                      dir:pos.add(pos2).scale(1/2),
                      life:500, planet:planetNDX});
  }
  ...
  for(part in particles)
  {
      ...
      var dist = pa.pos.dist(planets[pa.planet].position)-planets[pa.planet].radius + 100;		
      if(dist < 100)
        pa.life = 0;
      var gravity = grav_const*((planets[pa.planet].mass * player.mass)/Math.pow(dist,2));
      pa.dir = pa.dir.add(planets[pa.planet].position.vecTo(pa.pos).scale(gravity));
  ...
  if(message != undefined)
	...
	redraw();
	setTimeout( function(){	 updateGame(); }, 0);
};